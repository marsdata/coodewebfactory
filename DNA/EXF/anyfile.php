...
if (($_COOKIE["deadline"]*1-time())<60*20){
     ...
     setcookie("gid",$_COOKIE["gid"],time()+3600,"/");
...
$flname=_get("filename");
$flpath=_get("filepath");
$ftxt=tostring(UX("select filetext as result from coode_sitefile where filename='".$flname."' or updateurl='".$flpath."'"));
if ($ftxt!=""){
}else{
  if (file_exists($flpath)){
    $ftxt=file_get_contents($flpath);
  }else{
    if (strpos("x".$flpath,"http")>0){
      $ftxt=file_get_contents($flpath);
    }else{
      $ftxt="";
    }
