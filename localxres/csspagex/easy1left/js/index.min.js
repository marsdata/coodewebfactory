...
function hideLeft(){
    ...
    var o2=$("#right-body");
    if($(o).is(":visible")){
        o2.animate({left:0,width:'100%'},100,function(){
            $(o).hide();
        });
        ...
        o2.animate({left:200,width:$(document).width()-200},100,function(){
            $(o).show();
        });
...
function addTbar2(setting){
    ...
    if(tbarBox.length >= 20){
        LAY_UTIL.alertMessage("打开的tab页超过最大上限"+20);
        return;
    }
    ...
    if(st.clazz != undefined && st.clazz.length > 0){
        newTitle = "<i style='color:#aaa;line-height: 35px;font-size: 14px;margin-right:4px;font-style: normal'>×</i>" + st.title;
    }else{
        newTitle = st.title;
    }
    var html='<div class="tab-item selected-tab" id="tbarItem_'+id+'">'+
        '<div class="tab-title" title="'+st.title+'" onclick="switchTbar('+id+')">'+newTitle+'</div>';
    html+='<div class="tab-close"><a href="javascript:void(0)" onclick="closeTbar('+id+')">';
    if(st.canClose){
        html+="<i style='font-style: normal'>×</i>";
    }else{
        html+='<i style="font-style: normal">×</i>';
    }
    html+='</a></div></div>';
    ...
    $(obj).contextmenu({id:id},function(e){
        ...
        $("#contextmenu #loveIt").unbind("click");
        //$("#contextmenu #loveIt").click(hideTop);
...
function addIframe(id,url,param,method){
    if(url.indexOf('http:')!==0 && url.indexOf('https:')!==0){
        ...
        if(url.indexOf(root)!==0&& url.indexOf('../')!==0){
            url = root+url;
        }*/
    }
...
//        }
    $(".zUI-iframe").append('<iframe src="" id="iframe_'+id+'" name="iframe_'+id+
...
function switchTbar(id){
    ...
    for(var i=0;i<tbarBox.length;i++){
        if(tbarBox[i].id!=id){
            $("#tbarItem_"+tbarBox[i].id).removeClass("selected-tab");
    ...
    if(s){
        $("#tbarItem_"+id).addClass("selected-tab");
        showIframe(id);
    }
}
/**
...
function refreshTbar(id,newParam){
